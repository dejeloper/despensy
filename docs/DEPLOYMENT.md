...
5.  Sube por FTP: - Se conecta y navega a la raíz de la app (`DEPLOY_FTP_REMOTE_DIR`), luego llama a `client.clearWorkingDir()` — **borra todo el contenido remoto** (el despliegue anterior completo) antes de subir nada. - `production.zip` y `deploy.token` → raíz de la app. - `install.php` → subcarpeta `public/` (se recrea tras el borrado).
...
6.  Build: `config:cache`, `route:cache`, `view:cache`, `optimize:clear`, `composer install --no-dev --optimize-autoloader`, `pnpm install`, `pnpm run build`.
7.  Copia selectiva a `production/`. Se excluye todo lo que no corre en producción: `node_modules`, `tests`, `.git`, `.agents`, `.claude`, `docs`, `.github`, `.vscode`, la config de tooling, y **todos los `.md`** (incluidos los README dentro de `vendor/`).
8.  Renombra `.env.prod` → `.env` dentro del paquete y comprime en `production.zip`.
9.  Genera un token con `crypto.randomBytes(24)` y lo escribe en `deploy.token`.
10. Sube por FTP: - `production.zip` y `deploy.token` → raíz de la app (`DEPLOY_FTP_REMOTE_DIR`). - `install.php` → subcarpeta `public/`.
    > > > > > > > a85048423556265be22ae4bc9f321d0a6c14ceae
11. Imprime la URL final: `https://<host>/install.php?token=<token>`. El token local se borra tras subirlo.
...
5. **Validaciones**: existe `production.zip`, existe `ZipArchive`, la raíz es escribible, y el ZIP abre correctamente (se valida **antes** de limpiar, para no dejar el sitio vacío si el ZIP está corrupto).
6. **Limpia la raíz**: borra todo el contenido de la raíz de la app para un despliegue limpio, conservando solo `production.zip`, `deploy.token` y el propio instalador (`public/install.php`).
7. **Extrae** `production.zip` a `$root/production/` y **valida la estructura** (existen `app`, `bootstrap`, `config`, `public`, `vendor`).
    > > > > > > > a85048423556265be22ae4bc9f321d0a6c14ceae
8. **Mueve** todo de `$root/production/` a `$root/`, con `rename` (rápido) y fallback a copia recursiva con fusión cuando el destino ya existe.
9. **Recrea `public/storage`** apuntando a `storage/app/public` (equivale a `php artisan storage:link`).
10. **Limpieza y auto-eliminación**: elimina `production/`, `production.zip`, `deploy.token` y `install.php`. No queda ningún archivo de instalación en el servidor.
