...
- Un Service es obligatorio cuando existe alguna de estas condiciones:
    - Hay una **query compuesta** (joins, subqueries, agregaciones) — ejemplo real: el cálculo de "última compra" de un producto, extraído a `ProductLastPurchaseService` y reutilizado por `ProductController` y `DespensyController`.
