...
- `app/Services/business/` para los Services de dominio (carpeta a crear — hoy no existe ninguna).

## Controladores

- Métodos en el orden convencional de un resource controller: `index, create, store, edit, update, destroy` (+ métodos de acción específicos al final, como `updatePrice`).
- `index()` no arma queries con joins/subqueries — eso se delega a un Service (ver `docs/ARCHITECTURE.md` para cuándo es obligatorio).
- Nunca `$request->validate([...])` inline (visto hoy en `ProductController::updatePrice`) — siempre un Form Request dedicado, aunque sea para una acción parcial (`ProductPriceRequest`, por ejemplo).
...
- Las reglas deben validar exactamente las columnas que existen en la migración — antes de escribir una regla para un campo, confirmar que el campo existe en `database/migrations/`. (Ver `docs/TECH_DEBT.md`: `ProductRequest` hoy valida `price`/`stock`, que no existen en la tabla.)
...
- Accessors (`getXAttribute`) solo para cómputos triviales de una fila (ej. formatear, concatenar). Cualquier cómputo que implique otra tabla o agregación va en un Service, no en un accessor (ejemplo de lo que NO repetir: `hasLowStock()`/`getTotalValueAttribute()` en `Product`, que además dependen de columnas inexistentes).
...
- El Resource se **usa** en el Controller (`ProductResource::collection(...)` o `ProductResource::make(...)`) — no se escribe un Resource que nunca se invoca, como pasa hoy con los cuatro Resources existentes.
...
- Indentación con 4 espacios, igual que la mayoría de migraciones existentes (ver `docs/TECH_DEBT.md` para las que hoy usan tabs).
...
- Los tests de modelo (`tests/Unit/Models/business/`) verifican relaciones, scopes y casts — patrón ya usado para Category/Place/Product/Unit, se mantiene para State/Checklist/ChecklistItem.
- Todo Service nuevo con lógica de negocio (ver `docs/ARCHITECTURE.md`, sección "cuándo crear un Service") lleva su test unitario antes de darse por terminado — hoy no existe ningún test de Service porque no existe ningún Service; esto aplica desde el primero que se cree.
- Los flujos de controlador con reglas de negocio (ej. "no se puede crear una segunda checklist abierta") llevan un Feature test — hoy no hay Feature tests para controladores de negocio, es deuda a cerrar según se construya ese código (`docs/TECH_DEBT.md`).
